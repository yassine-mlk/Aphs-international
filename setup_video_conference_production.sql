-- =========================================
-- CONFIGURATION VIDÉOCONFÉRENCE PRODUCTION
...
-- Activer Realtime pour video_meetings
ALTER PUBLICATION supabase_realtime ADD TABLE video_meetings;

-- Activer Realtime pour video_meeting_participants
ALTER PUBLICATION supabase_realtime ADD TABLE video_meeting_participants;

-- Activer Realtime pour video_meeting_requests
ALTER PUBLICATION supabase_realtime ADD TABLE video_meeting_requests;

-- =========================================
-- 2. CRÉER LES TABLES SI ELLES N'EXISTENT PAS
-- =========================================

-- Table des réunions vidéo
...
CREATE TABLE IF NOT EXISTS video_meeting_requests (
    ...
    scheduled_time TIMESTAMP WITH TIME ZONE,
    participants TEXT[], -- IDs des participants suggérés
...
-- Fonction pour ajouter un participant à une réunion
...
DROP POLICY IF EXISTS "Admins can manage meeting requests" ON video_meeting_requests;
CREATE POLICY "Admins can manage meeting requests" ON video_meeting_requests
    FOR UPDATE USING (
        EXISTS (
            SELECT 1 FROM profiles 
            WHERE user_id = auth.uid() AND role = 'admin'
...
SELECT 
    schemaname,
    tablename,
    CASE WHEN schemaname = 'public' AND tablename IN ('video_meetings', 'video_meeting_participants', 'video_meeting_requests')
         THEN '✅ Realtime activé'
         ELSE '❌ Realtime non activé'
    END as realtime_status
FROM pg_tables 
WHERE tablename IN ('video_meetings', 'video_meeting_participants', 'video_meeting_requests');
...
SELECT 'Configuration vidéoconférence production terminée avec succès !' as status; 