  ...
  const initializeLocalStream = useCallback(async () => {
    try {
      ...
      if (localVideoRef.current) {
        console.log('🎥 Attaching local stream to video element (WebRTCMeeting_v2)...', stream);
        localVideoRef.current.srcObject = stream;
        localVideoRef.current.muted = true;
        
        // Forcer la lecture de la vidéo
        localVideoRef.current.play().then(() => {
          console.log('✅ Local video playing successfully (WebRTCMeeting_v2)');
        }).catch(error => {
          console.warn('⚠️ Could not auto-play local video (WebRTCMeeting_v2):', error);
        });
        
        console.log('✅ Local stream attached to video element (WebRTCMeeting_v2)');
      }
