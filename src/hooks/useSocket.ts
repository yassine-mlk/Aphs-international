...
import { useRealtimeParticipants } from './useRealtimeParticipants';
...
export function useSocket({ roomId, userName, userId }: UseSocketProps) {
  ...
  const realtimeParticipants = useRealtimeParticipants({ roomId, userName });
  const participants = realtimeParticipants.participants;

  useEffect(() => {
    console.log(`🔌 Initializing socket for room: ${roomId}, user: ${userName} (${userId})`);
    
    // Priorité à Supabase Realtime, fallback vers localStorage en cas d'échec
    const useRealtimeFallback = import.meta.env.VITE_USE_REALTIME !== 'false';
    
    // Fonction pour initialiser le mode localStorage
    const initializeLocalStorageMode = () => {
      ...
      const cleanupJoin = simulatedSocket.on('join', (data) => {
        if (data?.userId && data.userId !== userId) {
          // Les participants sont maintenant gérés par useRealtimeParticipants
          console.log(`👋 User joined room ${roomId}: ${data.userId}`);
        }
      });
      ...
      const cleanupLeave = simulatedSocket.on('leave', (data) => {
        if (data?.userId) {
          // Les participants sont maintenant gérés par useRealtimeParticipants
          console.log(`👋 User left room ${roomId}: ${data.userId}`);
        }
      });
    ...
    // Vérification plus robuste pour éviter l'erreur undefined et essayer Supabase en premier
    if (useRealtimeFallback && supabaseHook && supabase) {
      try {
        ...
        return initializeLocalStorageMode();
      }
    }

    // Si Realtime est désactivé ou pas disponible, utiliser localStorage directement
    return initializeLocalStorageMode();
